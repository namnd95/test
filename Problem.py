...
class Problem:
    ...
    VALUE = {'True':True, 'False':False, 
        ...
        'equal':Judge.equal}
    DEFAULT_CONFIG = {'stdin':'True', 'stdout':'True',                          #Need change
    ...
    def loadConfig(self, createFiles):
        try:
            ...
            if createFiles: self.writeConfig()
    ...
    def writeTestcase(self):
        ...
        for value in self.default_case: fileTestcase.write('%s ' % value)
        fileTestcase.write('\n')
        ...
        for id_case, case in self.testCases.iteritems():
            ...
            for i in range(len(case)):
                d = case[i]
                if d==self.default_case[i]: d=self.DEFAULT
    ...
    def loadTestcase(self, createFiles):
        try:
            ...
            for i in xrange(2, len(cur)): cur[i] = float(cur[i])
            self.default_case = cur; 
            for line in fileTestcase:
                cur = line.split(); 
                id = cur.pop(0)
                for i in range( len(cur) ):
                    if cur[i]==self.DEFAULT: cur[i] = self.default_case[i]
                for i in xrange(2, len(cur)): cur[i] = float(cur[i])             
            ...
            if len(subDir) == 0:
                ...
                for file in listFile:
                    if isFileOut(file): fileOut.append(file)
                    elif not isNotFileIn(file): fileIn.append(file)
                fileIn.sort(); fileOut.sort()            
                ...
                for dir in subDir:     #name of testCases
                    ...
                    fileIn=ListFile[0];fileOut=ListFile[1];
                    if isFileOut(FileIn):fileIn, fileOut = fileOut, fileIn
                    self.testCases[dir] = [dir+'/'+FileIn, dir+'/'+FileOut]
                    for i in xrange(2,len(self.default_case)): 
                        self.testCases[dir].append(self.default_case[i])
            if createFiles: self.writeTestcase()

if __name__ == "__main__":    
    x = Problem('auction','D:/AMM2/Contests/SPOJ/Exams/auction/')
    print x.compare('','file.inp','file.out')
    print x.VALUE;
    for key,value in x.testCases.iteritems(): print key,value
    print x.subtask
