...
class Submission:
    ...
    def process(self):
        ...
        for id_case, case in self.problem.testCases.iteritems():
            ...
            exitCode = self.language.run(
                case[2], case[3], self.problem.stdin, self.problem.stdout)
            if exitCode != 0:
                if (exitCode == 1):
                    exitCode = -9  # Windows exit code to Unix
                if exitCode > 0:
                    exitCode = -exitCode
                score[id_case] = exitCode
            else:
                result = self.problem.compare(dir + case[0],
                                              self.problem.default_case[
                                                  1], dir + case[1],
                                              self.problem.directory)
                score[id_case] = result
            if score[id_case] <= 0 and self.problem.canStop:
                break
            print 'finish %s' % id_case

        cleanUp()
        return self.problem.calculateScore(
            score, self.problem.testCases, self.problem.subtask)
...
if __name__ == "__main__":
    ...
    while True:
        ...
        prob, language, location = [
            'game', 'cpp', 'D:/Problem/greedy/game/game.cpp']
